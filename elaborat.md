...
Последната фаза го применува манифестите на вистински кластер (Vultr + k3s) и го врши rollout-от на новоизградените слики. Активна е само ако постои `KUBE_CONFIG` secret и ако API-то на кластерот е достапно (има брза проверка на достапност од 15 секунди); во спротивно фазата уредно се прескокнува без да го сруши pipeline-от.
...
Pipeline-от содржи `deploy` фаза подготвена за вистински кластер. Бидејќи локалниот k3d кластер не е достапен од GitHub-овите runner-и, bonus CD е реализиран на **Vultr cloud инстанца** (Ubuntu + k3s):
