...
3. [Git репозиториум (10%)](#3-git-репозиториум-10)
4. [Докеризација на апликацијата (10%)](#4-докеризација-на-апликацијата-10)
5. [Оркестрација со Docker Compose (10%)](#5-оркестрација-со-docker-compose-10)
6. [CI/CD pipeline со GitHub Actions (20%)](#6-cicd-pipeline-со-github-actions-20)
7. [Kubernetes манифести](#7-kubernetes-манифести)
8. [Демонстрација (посебен namespace + докажување дека работи) (10%)](#8-демонстрација-посебен-namespace--докажување-дека-работи-10)
...
## 4. Докеризација на апликацијата (10%)
...
## 5. Оркестрација со Docker Compose (10%)
...
## 6. CI/CD pipeline со GitHub Actions (20%)
...
### 6.5 Фаза 4 — CD (опционален, bonus)

Последната фаза го применува манифестите на вистински кластер и го врши rollout-от на новоизградените слики. Активна е само ако постои `KUBE_CONFIG` secret и ако API-то на кластерот е достапно (има брза проверка на достапност од 15 секунди); во спротивно фазата уредно се прескокнува без да го сруши pipeline-от.
...
### 7.2 Deployment за апликацијата (10%)
...
`ml` и `frontend` Deployment-ите следат иста шема, со HTTP health probes (`/health` за `ml`, `/` за `frontend`).

### 7.3 Service за апликацијата (10%)
...
### 7.4 Ingress за апликацијата (10%)
...
Бидејќи и frontend-от и API-то се на истиот хост (`radiosignals.local`), нема проблеми со CORS — прелистувачот ги праќа сите повици кон истиот origin.

### 7.5 StatefulSet за базата со ConfigMaps/Secrets (10%)
...
## 8. Демонстрација (посебен namespace + докажување дека работи) (10%)
...
Pipeline-от веќе содржи `deploy` фаза подготвена за вистински кластер. Бидејќи локалниот k3d кластер не е достапен од GitHub-овите runner-и, како bonus е предвидено распослање на **бесплатна cloud инстанца** — Oracle Cloud „Always Free" ARM сервер со **k3s**:

1. Се подига ARM инстанца (4 OCPU / 24 GB, бесплатно) со јавна IP адреса.
2. Се инсталира k3s со `--tls-san <PUBLIC_IP>` (за валиден TLS сертификат).
3. Се отвораат портите 6443/80/443 (OCI Security List + iptables на самата машина).
4. kubeconfig-от (со заменета јавна IP) се чува како GitHub Secret `KUBE_CONFIG`.
5. Сликите се градат **multi-arch** (`linux/amd64` + `linux/arm64`) за да работат на ARM јазолот.
...
- `deploy` фаза со автоматска проверка на достапност и уредно прескокнување.
- Custom имиња (`radio-signals` namespace, `radiosignals.local` хост, специфични имиња на сервиси) и организација преку `kustomize`.
